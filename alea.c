...
{	// CMS algorithm (J.M. Chambers, C.L. Mallows and B.W. Stuck)
	...
	//--------------------------------------------
	// Define k(¦Á) = 1 ? |1?¦Á|. Thus if ¦Á¡Ü1 then k(¦Á)=¦Á and if ¦Á¡Ý1 then k(¦Á)=2?¦Á.   
	...
	// Transform ¦Â to ¦Â' by ¦Â' = ¦Â if ¦Á=1 and ¦Â'= ?tan(?¦Ð(1?¦Á))tan(¦Á¦Õ0) otherwise.

	if(fabs(alpha-1)<zero) betaPrime=beta;
	else
		betaPrime=-tan(0.5*pi*(1-alpha))*temp;

	// Generate a random variable u uniformly distributed on the interval [0, 1] and compute ¦Õ = ¦Ð(u??). 		
	phi=pi*(alea(0,1)-0.5);

	//Compute ¦Å=1?¦Á and then ¦Ó = ?¦Åtan(¦Á¦Õ0) 		
	eps=1-alpha; 
	tau=-eps*temp;

	//Compute tan(?¦Õ), tan(?¦Å¦Õ) and tan(?¦Å¦Õ)/(?¦Å¦Õ). 
	t1=tan(0.5*phi);
	t2=tan(0.5*eps*phi);
	t3=2*t2/(eps*phi);
	...
	// Compute z = (cos(¦Å¦Õ)?tan(¦Á¦Õ0)sin(¦Å¦Õ)/(wcos(¦Õ))
	...
	// Compute s = tan(¦Á¦Õ0) + z^¦Å/¦Á (sin(¦Á¦Õ)?tan(¦Á¦Õ0)cos(¦Á¦Õ))/cos(¦Õ) 
